...
REM 停止 Firewall MCP 服务
echo [4/5] 停止 Firewall MCP 服务...
taskkill /FI "WINDOWTITLE eq Firewall MCP Server*" /F >nul 2>&1
if errorlevel 1 (
    echo [信息] Firewall MCP 服务未运行或已停止
) else (
    echo [成功] Firewall MCP 服务已停止
)
echo.
